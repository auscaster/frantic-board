#!/usr/bin/env bash
# Usage: ./verify/131-check-ausca-document-ocr.sh <evidence.json> <report.md> <receipt_ref>
# Verifies the machine floor for bounty 131: Run Ausca Document OCR end to end and report the process.
set -euo pipefail
...
# Check that report mentions Document OCR / extract-text / artifact operations
grep -Eiq '(document|ocr|extract-text|artifact|pdf|image|scan)' "$REPORT" \
  || fail "report lacks Document OCR, extract-text, or artifact references"
...
echo "PASS: Ausca Document OCR evidence packet, report, and receipt reference verified"
