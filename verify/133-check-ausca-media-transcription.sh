#!/usr/bin/env bash
# Usage: ./verify/133-check-ausca-media-transcription.sh <evidence.json> <report.md> <receipt_ref>
# Verifies the machine floor for bounty 133: Run Ausca Media Transcription end to end and report the process.
set -euo pipefail
...
# Check that report mentions Media Transcription / transcribe / media / audio / artifact operations
grep -Eiq '(transcription|transcribe|media|audio|video|speech|language|transcript|artifact)' "$REPORT" \
  || fail "report lacks Media Transcription, transcribe, audio/video, or artifact references"
...
echo "PASS: Ausca Media Transcription evidence packet, report, and receipt reference verified"
