#!/usr/bin/env bash
# Usage: ./verify/135-check-ausca-agent-inbox.sh <evidence.json> <report.md> <receipt_ref>
# Verifies the machine floor for bounty 135: Run an Ausca Agent Inbox end to end and report the process.
set -euo pipefail
...
// Validate observation fields (each should note step/elapsed time)
...
# Check that report contains concrete findings/issues/mismatches
...
# Expected: https://runx.ai/r/<digest> or runx:receipt:sha256:<hash> or frantic:receipt:<id>
...
echo "PASS: Ausca Agent Inbox evidence packet, report, and receipt reference verified"
